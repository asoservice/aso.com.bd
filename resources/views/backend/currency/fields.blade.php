            ...
            <div class="image-list">
                <div class="image-list-detail">
                    <div class="position-relative">
                        <img src="{{ $currency->getFirstMedia('currency')->original_url }}" id="{{ $currency->getFirstMedia('currency')->original_url }}" alt="User Image" class="image-list-item">
...
<div class="form-group row">
    ...
    <div class="col-md-10 error-div select-dropdown">
        <select class="select-2 form-control" id="code" name="code" data-placeholder="{{ __('static.currency.select_code') }}">
            <option class="select-placeholder" value=""></option>
            @foreach ($code as $key => $option)
                <option class="option" value="{{ $key }}" @if (old('code', isset($currency) ? $currency->code : '') == $key) selected @endif>{{ $option }}</option>
            @endforeach
        </select>
        @error('code')
        <span class="invalid-feedback d-block" role="alert">
...
<div class="form-group row">
    ...
    <div class="col-md-10">
        <input class='form-control' type="text" id="symbol" name="symbol" value="{{ isset($currency->symbol) ? $currency->symbol : old('symbol') }}" placeholder="{{ __('static.currency.enter_symbol') }}" readonly>
        @error('symbol')
            <span class="invalid-feedback d-block" role="alert">
                <strong>{{ $message }}</strong>
            </span>
...
<div class="form-group row">
    ...
    <div class="col-md-10">
        <input class='form-control' id="no_of_decimal" type="number" name="no_of_decimal"  value="{{ isset($currency->no_of_decimal) ? $currency->no_of_decimal : old('no_of_decimal') }}" placeholder="{{ __('static.currency.enter_number_of_decimal') }}">
        @error('no_of_decimal')
            <span class="invalid-feedback d-block" role="alert">
                <strong>{{ $message }}</strong>
            </span>
...
<div class="form-group row">
    ...
    <div class="col-md-10">
        <input class='form-control' type="number" name="exchange_rate" id="exchange_rate" value="{{ isset($currency->exchange_rate) ? $currency->exchange_rate : old('exchange_rate') }}"  placeholder="{{ __('static.currency.enter_exchange_rate') }}">
        @error('exchange_rate')
            <span class="invalid-feedback d-block" role="alert">
                <strong>{{ $message }}</strong>
            </span>
