...
@section('title', __('static.service_request.service_requests'))

@section('content')
    <div class="card">
        <div class="card-header d-flex align-items-center">
            <h5>{{ __('static.service_request.service_requests') }}</h5>
            <div class="btn-action">
                {{-- @can('backend.service.create') --}}
                    <div class="btn-popup ms-auto mb-0">
                        <a href="{{ route('backend.service-requests.create') }}" class="btn">{{ __('static.service_request.create') }}</a>
                    </div>
                {{-- @endcan --}}
                <a href="javascript:void(0);" class="btn btn-sm btn-secondary deleteConfirmationBtn" style="display: none;"
                    data-url="{{ route('backend.delete.serviceRequest') }}">
